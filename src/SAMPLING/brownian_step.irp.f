...
! enddo
 ...
 BEGIN_PROVIDER [ double precision, drift_renorm, (elec_num) ]
 implicit none
 BEGIN_DOC
 ! Regularized drift, Eq(33) or Umrigar 93
 END_DOC
 double precision :: tau_v2, f
 integer :: i

 do i=1,elec_num
   tau_v2 = time_step * (psi_grad_psi_inv_x(i)*psi_grad_psi_inv_x(i) + &
                         psi_grad_psi_inv_y(i)*psi_grad_psi_inv_y(i) + &
                         psi_grad_psi_inv_z(i)*psi_grad_psi_inv_z(i) )
   drift_renorm(i) = (dsqrt(1.d0 + 2.d0*tau_v2)-1.d0)/tau_v2
 enddo
...
subroutine brownian_step_reg(p,q,accepted,delta_x)
  ...
  !DIR$ VECTOR ALIGNED
  bold0_x = psi_grad_psi_inv_x
  bold_x = psi_grad_psi_inv_x * drift_renorm
  !DIR$ VECTOR ALIGNED
  bold0_y = psi_grad_psi_inv_y
  bold_y = psi_grad_psi_inv_y * drift_renorm
  !DIR$ VECTOR ALIGNED
  bold0_z = psi_grad_psi_inv_z
  bold_z = psi_grad_psi_inv_z * drift_renorm
...
! enddo
  ...
  !DIR$ VECTOR ALIGNED
  bnew_x = psi_grad_psi_inv_x * drift_renorm
  !DIR$ VECTOR ALIGNED
  bnew_y = psi_grad_psi_inv_y * drift_renorm
  !DIR$ VECTOR ALIGNED
  bnew_z = psi_grad_psi_inv_z * drift_renorm
